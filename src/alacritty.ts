...
export function fontToml(family: string, size: number): string {
  // FiraCode deliberately publishes no italic faces. Asking Alacritty
  // for one makes font matching fall through to another family, so an
  // italic span changes typeface in the middle of the terminal. Keep
  // the chosen family and use its real upright faces instead.
  const hasItalics = family !== "FiraCode Nerd Font Mono";
  const italic = hasItalics ? "Italic" : "Regular";
  const boldItalic = hasItalics ? "Bold Italic" : "Bold";
...
async function shellToml(p: Platform): Promise<string> {
  const version = await installedAlacrittyVersion(p);
  // Linux still needs an explicit choice. Inheriting the account's login
  // shell makes the managed bash -> Zellij -> ble.sh stack disappear on
  // machines whose login shell is fish or zsh.
  if (p.os !== "windows" && p.env !== "wsl") {
    return `# Generated by red-dev -- the managed terminal shell.${linuxBashShellSection(version)}`;
  }
...
`;
  return header + (await shellSectionFor(p, version));
}

function shellTable(version?: AlacrittyVersion | null): "shell" | "terminal.shell" {
  return version && version[0] === 0 && version[1] < 14 ? "shell" : "terminal.shell";
}

/** Launch the bash whose rc hands the terminal to Zellij. */
export function linuxBashShellSection(version?: AlacrittyVersion | null): string {
  return `
 ...
 */
async function shellSectionFor(
  p: Platform,
  ...
  if (choice === "wsl") {
    ...
    return wslShellSection(distro, version);
  }

  return gitBashSection(version);
}

function gitBashSection(version?: AlacrittyVersion | null): string {
  const candidates = [
  ...
  return gitBashShellSection(found, version);
}

/** Launch a WSL distro, starting in its home rather than the slow /mnt/c. */
export function wslShellSection(
  distro: string,
  version?: AlacrittyVersion | null,
): string {
  return `
...
export function gitBashShellSection(
  path: string,
  version?: AlacrittyVersion | null,
): string {
  return `
...
/**
 * One entry in Alacritty's keyboard table for a gesture from the registry.
 *
 * `chars` rather than an action, because what these two bindings do is
 * send bytes: src/actions/input.ts holds the sequence, and this writes it
 * in the escaped spelling a TOML file can carry. Nothing here decides
 * which bytes — a second module deciding that is the drift this replaced.
 */
function sequenceBinding(binding: InputBinding): string {
  return [binding.layers.alacritty, ...(binding.layers.alacrittyAliases ?? [])]
    .map((layer) => `[[keyboard.bindings]]
key = '${layer.key}'
mods = '${layer.mods}'
chars = "${escapedSequence(binding.sequence)}"`)
    .join("\n\n");
}
 ...
 */
export function keysToml(): string {
  return `# Generated by red-dev -- terminal keys.
#
# REWRITTEN ON EVERY CONVERGE. Bindings added here are lost the next time
# red-dev runs. Put your own in alacritty.toml, which is created once and
# never rewritten; Alacritty merges both, and a binding there wins.
#
# This warning exists because it was missing. The sentence was in
# alacritty.toml -- "the imported files, which red-dev regenerates" --
# which is the wrong file to say it in: nothing in a file named keys.toml
# suggested that editing keys was the thing not to do here.
#
# Ctrl+V is deliberately not bound here. Claude Code and Codex consume
# its raw control byte to attach an image from the system clipboard; an
# Alacritty Paste action would swallow the key before either agent saw it.
# Ctrl+Shift+V remains Alacritty's built-in terminal text-paste binding;
# repeating it here would execute Paste twice for one key press.
#
# Alt+V sends the same byte as an alias and matches Claude Code's
# documented Windows image shortcut.

${sequenceBinding(IMAGE_PASTE_INPUT)}
...
# A second window, declared rather than inherited.
#
# Alacritty binds this itself on Linux and BSD, and a machine that had
# it was relying on that default -- which is not a promise the four
# targets keep equally, and not one this project should be quiet about.
# Declared here it behaves the same on Ubuntu, WSL and native Windows.
 ...
 *
 * Absolute paths throughout. Alacritty 0.13 resolves a relative import
 * against the process working directory, not against alacritty.toml. A
 * launcher started from anywhere else then silently loses the generated
 * shell and key files. Windows paths stay literal TOML strings so their
...
export function requiredImports(
  shareWinRoot: string | null,
  localConfigDir?: string,
): string[] {
  const qualify = (dir: string | undefined, part: string): string => {
    if (!dir) return part;
    const clean = dir.replace(/[\\/]+$/, "");
    const separator = clean.includes("\\") ? "\\" : "/";
    return `${clean}${separator}${part}`;
  };
  const local = LOCAL_PARTS.map((part) => qualify(localConfigDir, part));
  if (!shareWinRoot) return [...SHARED_PARTS.map((part) => qualify(localConfigDir, part)), ...local];
...
export async function configureAlacritty(opts: AlacrittyOptions): Promise<void> {
  ...
  const required = requiredImports(shared?.win ?? null, winDir ?? dir);

  // cursor, font and keys are the same on every machine, so they go to
  // the share when there is one; the main file is written once, so what a
  // user puts THERE survives every run.
  //
  // The previous wording said bindings survived, which was false in the
  // one way that costs someone work: keys.toml is rewritten here
  // unconditionally, so a binding added to it is gone at the next
  // converge. The file now says so in its own header — the promise and
  // the behaviour disagreed, and the behaviour is the one that is right.

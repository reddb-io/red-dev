 ...
 *
 * On native Windows this is Git Bash, not PowerShell: it is the only
 * way the dotfiles this project ships actually apply, and bash on all
 * five targets is the whole point of standardising.
 */
function shellToml(p: Platform): string {
  const header = `# Generated by red-dev — which shell Alacritty launches.
#
# Alacritty has no profiles, so one config means one shell. On a machine
# that runs both native Windows and WSL against the same host Alacritty,
# whichever ran red-dev last wins. That is deliberate and predictable:
# run it from the environment you want the terminal to open into.
`;

  return header + shellSection(p);
}

function shellSection(p: Platform): string {
  if (p.os === "windows") {
    const candidates = [
...
export async function configureAlacritty(opts: AlacrittyOptions): Promise<void> {
  ...
  await Bun.write(`${dir}${sep}shell.toml`, shellToml(opts.platform));

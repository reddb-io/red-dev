...
/**
 * `serve`'s exit when there is nothing to serve — no package under mise,
 * or no node to run it. The unit lists it under RestartPreventExitStatus:
 * a restart cannot install a package, and a service that retries the
 * same missing file every five seconds is a journal full of the same
 * line and a machine that feels broken.
 */
export const ROUTER_EXIT_NOT_INSTALLED = 3;
 ...
 *
 * Bounded, twice. `RestartPreventExitStatus` names the exit `serve`
 * uses for "nothing here to run", which no restart can fix. The start
 * limit is for everything else: five failures in five minutes and
 * systemd stops trying, instead of restarting a broken server every
 * five seconds until someone notices the journal. The converge clears
 * the limit (`reset-failed`) before it starts the unit again, so a fix
 * takes effect at the next `red-dev install` and not at the next login.
 */
export function routerUnit(binary: string, inheritedPath = ""): string {
  return [
    ...
    "Restart=on-failure",
    "RestartSec=5s",
    `RestartPreventExitStatus=${ROUTER_EXIT_NOT_INSTALLED}`,
    "",
    "[Install]",
...
export interface RouterSeams {
  ...
  /** The installed package's directory, or null when mise has not put one down. */
  packageDir?: () => string | null;
}

async function packageOf(seams: RouterSeams, env: NodeJS.ProcessEnv): Promise<string | null> {
  if (seams.packageDir) return seams.packageDir();
  const { miseInstallRoot } = await import("./mise-config.ts");
  return locateRouterPackage(miseInstallRoot(env));
}
...
async function convergeUnit(
  ...
  await run(["systemctl", "--user", "reset-failed", ROUTER_SERVICE]);

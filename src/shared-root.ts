...
/**
 * Files the old root may hold that the new one deliberately does not.
 *
 * Both were written by red-dev and both have since been retired — the
 * twenty-value ANSI palette and the zellij theme that read from it, see
 * .red/adr/0003. Their absence next door is the correct state, not a
 * copy that failed, so the prune must not treat them as unaccounted for.
 */
const RETIRED_IN_LEGACY = new Set([
  "config/alacritty/theme.toml",
  "config/zellij/themes/red-dev.kdl",
]);
...
/**
 * What a prune of the old root would do, decided without touching disk.
 *
 * Pure so the interesting cases — a file the new root never received, a
 * file whose bytes differ — can be tested without two real directories
 * and a Windows profile.
 *
 * `superseded` is the whole safety argument: a file is only allowed to
 * be deleted if the new root has one with identical bytes, or if it is a
 * file this project has since retired on purpose. Anything else is
 * `unaccounted`, and one of those cancels the prune entirely rather than
 * deleting the rest around it — a half-pruned directory is harder to
 * reason about than an untouched one.
 */
export function pruneVerdict(
  legacy: { path: string; sha: string; generated?: boolean }[],
  current: Map<string, string>,
): { superseded: string[]; unaccounted: string[] } {
  const superseded: string[] = [];
  const unaccounted: string[] = [];
  for (const f of legacy) {
    // Retired on purpose: absent next door is the correct state.
    if (RETIRED_IN_LEGACY.has(f.path)) superseded.push(f.path);
    // Byte-identical: the copy plainly succeeded.
    else if (current.get(f.path) === f.sha) superseded.push(f.path);
    // Ours, and older.
    //
    // This is the case that made a strict byte match useless in
    // practice. The new root keeps being written — keys.toml gained a
    // Shift+Enter binding, bat's config changed marker, zellij's lost
    // its theme line — so within one release every generated file
    // differs, and the prune refuses forever on files nobody would miss.
    //
    // Only where the OLD copy says red-dev generated it, and only where
    // the new root has a file of the same name. config/bash/local.sh is
    // the counter-example the marker exists to protect: red-dev creates
    // it once, never rewrites it, and its header says "Yours."
    else if (f.generated === true && current.has(f.path)) superseded.push(f.path);
    else unaccounted.push(f.path);
  }
  return { superseded, unaccounted };
}

/**
 * Did red-dev write this file, as the file itself claims?
 *
 * Read from the head rather than inferred from the path, because the
 * question is about provenance and only the content can answer it. Two
 * spellings, both of which red-dev has shipped: the generated-file
 * marker, and the header on the dotfiles it installs once.
 *
 * "Yours." is checked first and wins. config/bash/local.sh is created by
 * red-dev and belongs to the user, and it names red-dev in its own first
 * line — so a looser match would delete the one file here that has never
 * been safe to delete.
 */
export function looksGenerated(text: string): boolean {
  const head = text.slice(0, 400);
  if (/^#\s*Yours\./m.test(head)) return false;
  return /Generated by red-dev/.test(head) || /^\/\/ red-dev —/m.test(head);
}

/**
 * Finish the move the namespace migration started.
 *
 * `2026-08-06-share-root-namespace` copied ~/.reddev into ~/.red/dev and
 * deliberately did not delete the source: the ledger's rule is that a
 * migration is never destructive, and at the time nothing had verified
 * the copy. That was right then and is wrong now — what it leaves is a
 * second share root that nothing reads, holding a stale copy of every
 * config, which is the first thing anyone finds when they go looking for
 * where their settings live.
 *
 * So this completes it, and only on proof: every file in the old root
 * must be byte-identical to its counterpart in the new one, or be a file
 * red-dev has since retired. A single unexplained file leaves the whole
 * directory alone and says which one it was. That is not the ledger rule
 * being broken — nothing is being destroyed, because every byte exists
 * next door and has been compared.
 */
export async function pruneLegacyRoot(p: Platform, root: SharedRoot): Promise<boolean> {
  const profile = windowsProfileOf(root.windows);
  if (!profile) return false;

  const legacyWin = legacyRoot(profile);
  // Only the default the project itself wrote. A root the user pointed
  // at .reddev on purpose is theirs, and namespaceMove already refuses
  // to relocate one of those for the same reason.
  if (legacyWin.toLowerCase() === root.windows.replace(/[\\/]+$/, "").toLowerCase()) return false;

  const legacyLocal = localPath(legacyWin, p.env);
  if (!existsSync(legacyLocal)) return false;

  const legacy: { path: string; sha: string; generated: boolean }[] = [];
  for (const rel of filesUnder(legacyLocal)) {
    const file = Bun.file(`${legacyLocal}/${rel}`);
    const bytes = await file.bytes();
    // Decoded lossily on purpose: this only ever reads a header, and a
    // binary in the share must answer "not generated" rather than throw.
    ...
    legacy.push({
      ...
      generated: looksGenerated(head),
    });
  }

  const current = new Map<string, string>();
  for (const rel of existsSync(root.local) ? filesUnder(root.local) : []) {
    const bytes = await Bun.file(`${root.local}/${rel}`).bytes();
    current.set(rel, new Bun.CryptoHasher("sha256").update(bytes).digest("hex"));
  }

  const { superseded, unaccounted } = pruneVerdict(legacy, current);

  if (unaccounted.length > 0) {
    log.warn(`${legacyWin} still holds ${unaccounted.length} file(s) not in the new root`);
    for (const f of unaccounted.slice(0, 3)) log.plain(`       ${f}`);
    log.plain(`       left alone — copy what you want, then remove it by hand`);
    return false;
  }

  rmSync(legacyLocal, { recursive: true, force: true });
  log.ok(`removed ${legacyWin} — ${superseded.length} file(s), all superseded`);
  return true;
}

/**
 * The Windows profile a recorded root sits under.
 *
 * Derived from the root rather than asked of Windows, because this runs
 * on both sides of the boundary and only one of them can answer
 * %USERPROFILE% without spawning a process.
 */
